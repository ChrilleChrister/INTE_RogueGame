...
public class TileGridMandelbrotIndexZeroZoomTwoTest extends TileGrid {
    ...
    private static final double ZOOM_IN_ON_MANDELBROT_TWO = 2.0;


    public TileGridMandelbrotIndexZeroZoomTwoTest() {
    ...
    protected double zoomOfMandelbrot() {
        return ZOOM_IN_ON_MANDELBROT_TWO;
    }



    @Test
    public void testMandelbrotIndexZeroZoom2() {
        Tile[][] grid = getWorldOfTiles();

        assertEquals(TileType.MOUNTAIN, grid[0][0].getTileType());
        assertEquals(TileType.SNOW, grid[0][1].getTileType());
        assertEquals(TileType.SNOW, grid[0][2].getTileType());
